CLASS zcl_adu_table_pack DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create
      IMPORTING iv_packet_size TYPE i
                it_table       TYPE ANY TABLE
      RETURNING VALUE(result)  TYPE REF TO zif_adu_table_pack
  ...
  PRIVATE SECTION.
    TYPES:
      ...
      END OF ty_line,
      ty_table TYPE SORTED TABLE OF ty_line WITH UNIQUE KEY index.

    DATA mv_packet_size TYPE i.
    DATA mv_iterator    TYPE i.
    DATA mt_table       TYPE ty_table.
    DATA mo_type        TYPE REF TO cl_abap_tabledescr.
...
CLASS zcl_adu_table_pack IMPLEMENTATION.
  ...
  METHOD create.
    ...
    result = NEW zcl_adu_table_pack( iv_packet_size = iv_packet_size
                                     it_table       = it_table ).
