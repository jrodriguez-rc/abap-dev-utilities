"! <p class="shorttext synchronized">Check transport</p>
CLASS zcl_adu_check_transport DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create
      IMPORTING transport_requests TYPE trkorrs
                rfc_source         TYPE rfcdest DEFAULT 'NONE'
                rfc_target         TYPE rfcdest
      RETURNING VALUE(result)      TYPE REF TO zif_adu_check_transport
  ...
  PRIVATE SECTION.
    ...
    DATA run_data                TYPE ts_run_data.
    DATA transport_requests      TYPE trkorrs.
    DATA results_cross_reference TYPE zif_adu_check_transport~tt_result_cross_reference.
    DATA results_sequence        TYPE zif_adu_check_transport~tt_result_sequence.
    DATA results_cross_release   TYPE zif_adu_check_transport~tt_result_cross_release.
    DATA results_import_time     TYPE zif_adu_check_transport~tt_result_import_time.
    DATA results_online_import   TYPE zif_adu_check_transport~ts_result_online_import.
    ...
    METHODS get_system_info
      IMPORTING rfcdest       TYPE rfcdest
      RETURNING VALUE(result) TYPE sysname
      RAISING   zcx_adu_check_transport.
...
CLASS zcl_adu_check_transport IMPLEMENTATION.
  ...
  METHOD create.
    ...
    result =
        NEW zcl_adu_check_transport( transport_requests = transport_requests
  ...
  METHOD zif_adu_check_transport~check_cross_reference.
    ...
    DATA requests TYPE STANDARD TABLE OF e070 WITH DEFAULT KEY.
    ...
    check_authorization( rfcdest          = run_data-rfc_source
                         authority_object = lc_authorization-source ).
    check_authorization( rfcdest          = run_data-rfc_destination
                         authority_object = lc_authorization-destionation ).

    requests =
        VALUE #( FOR transport_request IN transport_requests
  ...
  METHOD zif_adu_check_transport~check_sequence.
    ...
    DATA alog      TYPE tmstpalogs.
    DATA conflicts LIKE result.
  ...
  METHOD zif_adu_check_transport~check_cross_release.
    ...
    DATA alog          TYPE tmstpalogs.
    DATA cross_release LIKE result.
  ...
  METHOD zif_adu_check_transport~check_online_import.
    ...
    DATA requests      TYPE STANDARD TABLE OF e070 WITH DEFAULT KEY.
    DATA online_import LIKE result-all.
    DATA summary       LIKE result-summary.
  ...
  METHOD after_save.
    ...
    CLEAR run_data.
    CLEAR results_cross_reference.
    CLEAR results_sequence.
    CLEAR results_cross_release.
    CLEAR results_import_time.
    CLEAR results_online_import.
  ...
  METHOD get_system_info.
    ...
    DATA rfc_user     TYPE rfcuser.
    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA rfc_password TYPE rfcauth.
