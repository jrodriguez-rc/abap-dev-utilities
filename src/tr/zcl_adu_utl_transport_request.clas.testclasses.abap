...
CLASS ltcl_files DEFINITION FINAL
  ...
  PRIVATE SECTION.
    DATA:
      mi_utility TYPE REF TO zcl_adu_utl_transport_request.
...
CLASS ltcl_files IMPLEMENTATION.
  ...
  METHOD build_tr_from_filename_cofile.
    ...
    DATA(lv_transport_request) = mi_utility->convert_filename_to_tr( lc_file_cofile ).

    cl_abap_unit_assert=>assert_equals(
        act = lv_transport_request
        exp = lc_transport_request ).
  ...
  METHOD build_tr_from_filename_data.
    ...
    DATA(lv_transport_request) = mi_utility->convert_filename_to_tr( lc_file_data ).
  ...
  METHOD build_tr_from_filename_error.
    ...
    TRY.
        DATA(lv_transport_request) = mi_utility->convert_filename_to_tr( lc_file_not_supported ).
  ...
  METHOD filename_valid.
    ...
    cl_abap_unit_assert=>assert_true( mi_utility->is_filename_valid( `K9000000.A4H` ) ).
    cl_abap_unit_assert=>assert_true( mi_utility->is_filename_valid( `R9000000.A4H` ) ).
  ...
  METHOD filename_invalid.
    ...
    cl_abap_unit_assert=>assert_false( mi_utility->is_filename_valid( `X9000000.A4H` ) ).
    cl_abap_unit_assert=>assert_false( mi_utility->is_filename_valid( `R90000000.A4H` ) ).
    cl_abap_unit_assert=>assert_false( mi_utility->is_filename_valid( `SAP.A4H` ) ).

    cl_abap_unit_assert=>assert_false( mi_utility->is_filename_valid( `A4HK90000000` ) ).
    cl_abap_unit_assert=>assert_false( mi_utility->is_filename_valid( `A4HX9000000` ) ).
    cl_abap_unit_assert=>assert_false( mi_utility->is_filename_valid( `SAP.EXE` ) ).
