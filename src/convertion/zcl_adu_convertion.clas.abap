CLASS zcl_adu_convertion DEFINITION
  PUBLIC ABSTRACT
  CREATE PUBLIC.

  PUBLIC SECTION.

    INTERFACES zif_adu_convertion.

    ALIASES create_result_type FOR zif_adu_convertion~create_result_type.
    ALIASES move_results       FOR zif_adu_convertion~move_results.
  ...
  PROTECTED SECTION.
    ...
    METHODS build_structure
      ...
      RETURNING VALUE(ro_result) TYPE REF TO cl_abap_structdescr.

    METHODS build_table
      IMPORTING io_table         TYPE REF TO cl_abap_tabledescr
      RETURNING VALUE(ro_result) TYPE REF TO cl_abap_tabledescr.
    ...
    METHODS build_element
      ...
      RETURNING VALUE(ro_result) TYPE REF TO cl_abap_elemdescr.
...
CLASS zcl_adu_convertion IMPLEMENTATION.


  METHOD zif_adu_convertion~create_result_type.
  ...
  METHOD zif_adu_convertion~move_results.
    ...
    CASE io_element->type_kind.
      WHEN cl_abap_typedescr=>typekind_packed.
        ...
        IF mv_unix_timestamp = abap_true AND is_type_timestamp( io_element ).
          ro_result = cl_abap_elemdescr=>get_p( p_length   = 13
                                                p_decimals = 0 ).
        ELSE.
          ro_result = io_element.
        ENDIF.
  ...
  METHOD build_structure.
    ...
    ro_result = cl_abap_structdescr=>get( lt_components ).
  ...
  METHOD build_table.
    ...
    ro_result = cl_abap_tabledescr=>create( lo_line_new_type ).
  ...
  METHOD is_type_boolean.
    ...
    TRY.
        CAST cl_abap_elemdescr( io_type ).
  ...
  METHOD is_type_timestamp.
    ...
    rv_result = xsdbool(    io_type->get_relative_name( ) = 'TIMESTAMP'
    ...
    rv_result = xsdbool(    ls_ddic_header-refname = 'TZNTSTMPS'
