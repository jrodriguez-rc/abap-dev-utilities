CLASS ltcl_switch_test DEFINITION DEFERRED.
CLASS zcl_adu_switch DEFINITION LOCAL FRIENDS ltcl_switch_test.
CLASS ltcl_switch_test DEFINITION INHERITING FROM zcl_adu_switch FOR TESTING.

  PUBLIC SECTION.
    CLASS-METHODS create_test
      RETURNING
        VALUE(result) TYPE REF TO ltcl_switch_test.
...
CLASS ltcl_switch_test IMPLEMENTATION.
...
CLASS ltcl_simple DEFINITION FINAL
  ...
  PRIVATE SECTION.
    DATA:
      mo_switch TYPE REF TO ltcl_switch_test.
...
CLASS ltcl_simple IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    mo_switch = ltcl_switch_test=>create_test( ).
...
CLASS ltcl_condition_test DEFINITION FINAL.
...
CLASS ltcl_condition_test IMPLEMENTATION.
...
CLASS ltcl_condition IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    mo_switch->set_condition( NEW ltcl_condition_test( ) ).
  ...
  METHOD disabled.
    ...
    DATA(ls_data) = VALUE ltcl_condition_test=>ty_data( enabled = abap_false ).
  ...
  METHOD enabled.
    ...
    DATA(ls_data) = VALUE ltcl_condition_test=>ty_data( enabled = abap_true ).
